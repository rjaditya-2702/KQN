...
class KQN():
    def __init__(self, n_observations, n_actions):
        self.model = KAN(width=[n_observations,8,n_actions], grid=5, k=3, seed=0)
        
    def train(self, dataset, lr = 0.01, entropy = 10., opt = "LBFGS", epochs = 50, prune = False):
        self.model.train(dataset, opt, epochs, lr, entropy)
        if prune:
            self.model.prune()
...
class Runner():
    ...
    def train_inner_dqn(self): 
        ...
        try:
            next_states = torch.cat([torch.from_numpy(s) for s in batch.next_state if s is not None])
        ...
        pred_values = self.learner(torch.stack(state_batch)).gather(1, action_batch)
    ...
    def train_inner_kqn(self): 
        ...
        #Q(s, a) = reward(s, a) + Q(s_t+1, a_t+1)* gamma
        target_values = next_state_values * self.gamma + reward_batch
        dataset = torch.cat(state_batch, target_values)
        pred_values = self.learner.fit(dataset, opt='LBFGS', steps = 1, lamb = self.lr)
        loss = self.loss(pred_values, target_values.unsqueeze(1))
    ...
    def train(self, episodes=100, smooth=10): 
        ...
        for episode in range(episodes):
            ...
            for i in range(500): 
                ...
                self.memory.push(state, action, next_state, torch.tensor(reward))                
                
                if MODEL_TYPE == 'MLP':
                    loss = self.train_inner_dqn()
                else:
                    loss = self.train_inner_kqn()
                rewards += reward
...
def main(): 
    ...
    if MODEL_TYPE == 'MLP':
        ...
        dqn = KQN(env.observation_space.shape[0], env.action_space.n).to(device)
